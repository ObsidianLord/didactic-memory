...
import {Div, Caption, Panel, PanelHeader, PanelHeaderBack, Button, Group, CellButton, Header} from "@vkontakte/vkui";
...
export default class EditPodcast extends React.Component {
    ...
    async onMusicAddition() {
        const ws = this.state.waveSurfer;
        ws.pause()
        const audio = new Crunker();
        const music = await (await fetch('/sample_1.mp3')).arrayBuffer();
        const original_buffer = ws.backend.buffer;
        const result = await audio
            .fetchAudio('/sample_1.mp3')
            .then(buffers => {
                return audio.mergeAudio(buffers.concat(original_buffer))
            })

        console.log(result);
        // const newLength = ws.getDuration() - (rVal[0].end - rVal[0].start);
        const new_buffer = ws.backend.ac.createBuffer(original_buffer.numberOfChannels, ws.getDuration() * original_buffer.sampleRate, original_buffer.sampleRate);
        ...
        // new_buffer.copyToChannel(result, 0);
        // new_buffer.copyToChannel(result, 1);
